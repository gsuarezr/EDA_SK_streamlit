...
st.sidebar.title(
    "Different Parts of the Exploratory data analysis (EDA) conducted")

panel2 = st.sidebar.radio(
    "Select",
    ("Main Analysis", "Forecasting: Linear Regression", "Forecasting: Arima",
     "PCA", "Conclusions"),)
st.sidebar.markdown(
    "###### made by [Gerardo Suarez](https://mcditoos.github.io), You can find me on Github as [@mcditoos](https://github.com/mcditoos)"
)
if panel2 == "Main Analysis":
    panel2 = 0
    st.sidebar.title("Main analysis section to visualize:")
...
if panel2 == "Forecasting: Linear Regression":
    ...
    lag = st.slider("How many lag days?", 1, 21, 6)
    st.plotly_chart(
        get_trend_con(True, lag)
    )
if panel2 == "Forecasting: Arima":
...
if not panel2:
    ...
    if panel == "Patient Info":
        ...
        cut = st.slider("Where do you cut ouliers?", min_value=0,
                        max_value=500, step=10, value=32)
        option = st.radio(
            "Shift the minimum", ("False", "True"),
            horizontal=True)
        col1, col2 = st.columns(2)
        with col1:
            st.text("")
        ...
        st.plotly_chart(
            contact(cut=cut, confirmed=True, min=option),
            use_container_width=True)
        st.caption(
    ...
    if panel == "Search Trend":
        ...
        start_time = st.slider(
            ...
            value=(datetime.datetime(2019, 11, 1),
